...
|---|---|---|
| **2026-08-09** | **SESJA: DŁUG GRAFICZNY + STRONA AUTORA + KALKULATOR W BLOGU.** (1) **8 grafik Higgsfield** — wpisy 21/23/25 dostały po dwie śródtekstowe, metkowanie własne hero i OG (koniec kadrów z cudzych zasobów); prompty pisane jak brief art-directorski z wymuszonymi niedoskonałościami, żeby kadry nie czytały się jako CGI; trzy kadry poprawiane (zimny grading, zbyt luźny nesting, czytelny „lorem ipsum" na wszywce — rozmyty lokalnie, bo model go nie usunął); (2) **obce logo „THE TAILOR'S CO" skasowane z repo** po 3 miesiącach na `/uslugi/krojenie-wykonczenie.html`, zastąpione tą samą sceną z nadrukiem ISABELL (logo z `~/Desktop/logo.svg` jako referencja obrazowa); (3) **`/michal-jalbrzykowski.html` LIVE** — wizytówka ~360 słów, dwie odrzucone wersje po drodze (za bardzo „wpis blogowy", potem trzy zaprzeczenia kompetencji); (4) **atrybucja autorstwa domknięta w trzech miejscach**: schema `Person` 25/25, link z nazwiska w 25 wpisach, „Made by @mpjalbrzyk" w stopce 43 stron; (5) **baner kalkulatora w 15 wpisach** po audycie tematyki wszystkich 25, z wizualizacją interfejsu i mikro-dowodami; (6) **bugfixy:** `<a href>` wewnątrz atrybutu `alt`, przycisk-pastylka niezgodny z systemem (999 px → 8 px), specyficzność CSS zjadająca nagłówek banera; (7) **konwerter MDX załatany 4×** (hero, `author`, stopka, link autora) | `886ed68`, `c1cea82`, `23ee11e`, `509ed62`, `54f11df`, `5e0f85e` |
...
**Stan przed pit stopem 01.09 (po sesji 09.08):** fundament **domknięty**. Blog 25/25, wszystkie wpisy z własnymi grafikami, atrybucja autorstwa spójna, kalkulator wyeksponowany w 15 wpisach, strona autora live. **Do pit stopu nie ma już nic obowiązkowego do zrobienia na stronie** — zostały tylko rzeczy czekające na Michała (bio już dostarczone i wykorzystane, opinie Google, ewentualne wyniki) oraz terminy administracyjne (`validThrough` do 31.10, Gmail w Make do 12.10). Kolejny krok merytoryczny to **pomiar 01.09**, a dopiero po nim faza treści na wrzesień.
...
- 📍 **GOOGLE BUSINESS PROFILE: pierwszy odczyt 09.08, dane w `gsc-archiwum/2026-08-09_google-business-profile_szwalnia.md`.** Dwie zweryfikowane wizytówki na koncie: **ISABELL** (Trybunalska 4, Centrum Handlowe Nowy Głuchów, Kruszów — hurt/DTC) i **Szwalnia ISABELL** (Batorego 44, Ząbki — B2B). Rozdzielenie poprawne.
  - **Szwalnia, marzec–sierpień 2026:** 253 interakcje, **2 952 wyświetlenia profilu**, 755 unikalnych fraz, 896 wyświetleń miesięcznie, 2 opinie (5,0).
  - **Rozbicie interakcji: 179 tras (71%), 62 kliknięcia w witrynę (25%), 12 telefonów (5%).** Dominacja próśb o dojazd jest nietypowa dla B2B — do przemyślenia na pit stopie.
  - **Mapy Google to 20% wyświetleń** (437 mobile + 148 desktop). **Tego kanału GSC nie pokazuje w ogóle**, więc ruch lokalny jest w naszych raportach systematycznie niedoszacowany. Kliknięcia z wizytówki lądują w GA4 jako Direct albo Organic.
  - **Top frazy wizytówki:** szwalnia 308, isabell 70, producent 51, isabelle sukienki 49, isabella 43, sewing 42. Fraza „szwalnia" działa w wizytówce **lepiej niż na stronie** (w GSC 871 wyświetleń, pozycja 12, tylko 6 kliknięć). Osobno: **„szwalnia u olgi" 22 wyświetlenia** — ruch z wyszukiwania konkurenta.
  - ⚠️ **DO NAPRAWY W WIZYTÓWCE: godziny pokazują otwarcie o 06:00**, a NAP kanoniczny to **8:00–15:00**. Szóstka to wewnętrzny start produkcji, którego zgodnie z decyzją z 24.07 **nie komunikujemy klientom**.
  - ⚠️ **Zdjęcia w wizytówce: ostatnie sprzed 136 dni.** Google sam zgłasza to jako zaniedbanie.
  - ⚠️ **Panorama Firm ma stary telefon 660 142 562** zamiast 730 851 555 — niespójność NAP w katalogu zewnętrznym.
  - ℹ️ **Nie ma integracji API z Business Profile** i nie da się jej po prostu włączyć: Google wymaga osobnego wniosku o dostęp, rozpatrywanego ręcznie tygodniami. Dostęp realizowany przez przeglądarkę na zalogowanym koncie Michała.
...
1. 📋 **Wpisy blogowe: 23 z 25 opublikowane, zostały DWA.** Plan skrócony z 27 do 25 decyzją Michała 08.08.
   ...
   ✅ **DŁUG GRAFICZNY SPŁACONY 09.08 — 8 grafik, wszystkie live.** Wpisy 21/23/25 mają po dwie grafiki śródtekstowe, metkowanie dostało własne hero + OG.
   - `/rodzaje-sciegow-maszyny-szwalnicze.html`: `overlock-renderka-roznica-wykonczenia.webp` (makro: podwinięcie renderką obok krawędzi overlockowej) + `overlock-czteronitkowy-dzianina-obrzucanie.webp`.
   - `/metkowanie-odziezy-co-musi-byc-na-metce.html`: **hero własne** `metka-zakardowa-wszywka-konserwacji-hero.webp` (koniec kadru z cudzego zasobu), OG **pod nową nazwą** `og-metkowanie-odziezy-metki.jpg` + `metka-glowna-wszywka-boczna-podzial.webp` + `metkowanie-zawieszka-gotowe-wyroby.webp`.
   - `/szwalnia-z-wlasna-krojownia.html`: `uklad-kroju-marker-wykorzystanie-tkaniny.webp` + `relaksacja-dzianiny-przed-krojeniem.webp`.
   - ✅ **OBCE LOGO USUNIĘTE.** `assets/metkowanie-pakowanie-odziez.webp` („THE TAILOR'S CO" na czterech pudełkach, wisiało od maja) **skasowane z repo**. Na jego miejsce `metkowanie-pakowanie-odziez-isabell.webp` — ta sama scena, wygenerowana jako edycja oryginału z podmienionym nadrukiem na **logo ISABELL**. Źródło logo: `~/Desktop/logo.svg` (SVG z osadzonym PNG 377×82, fiolet) → przerobione na czarne i użyte jako referencja obrazowa w Higgsfieldzie. **Logo wolno używać na metkach, zawieszkach i pudełkach w generowanych grafikach** (decyzja Michała 09.08) — to wyjątek od reguły „bez tekstu i logo w kadrze".
   - ⚠️ **Nowe nazwy plików są obowiązkowe przy podmianie grafiki.** `vercel.json` daje obrazom w `/assets/` `max-age=31536000, immutable`, więc nadpisanie pliku pod starą nazwą nie dotrze do przeglądarek ani do scraperów social.
   - 🐞 **Naprawiony przy okazji:** `uslugi/krojenie-wykonczenie.html` miał znacznik `<a href>` **wewnątrz atrybutu `alt`** (skrypt linkujący trafił w złe miejsce) — link przeniesiony do treści karty, `alt` opisowy. Sprawdzone: to był jedyny taki przypadek w repo.
   - 🐞 **Pułapka konwertera potwierdzona i załatana:** `Wpisy-blogowe/convert_mdx.py` trzymał w mapowaniu **stary** hero metkowania i przy regeneracji wpisu cofnąłby zmianę. Poprawione u źródła.

2. ✅ **SESJA 09.08 — WSZYSTKIE CZTERY PUNKTY ZAMKNIĘTE.** Commity: `886ed68` (grafiki + obce logo), `c1cea82` (strona autora + atrybucja), `23ee11e` (grafika bouclé), `509ed62` (przycisk systemowy), `54f11df` + `5e0f85e` (baner kalkulatora). Łącznie **61 plików, +1194 linii**.
   - ✅ **Grafiki Higgsfield: ZROBIONE 09.08** (8 sztuk, szczegóły w punkcie wyżej).
   - ✅ **Obce logo: USUNIĘTE 09.08** (szczegóły w punkcie wyżej).
   - ✅ **STRONA AUTORA LIVE 09.08: `/michal-jalbrzykowski.html`.** Krótka wizytówka (~360 słów), **świadomie nie landing i nie druga wersja `o-nas`** — Michał odrzucił pierwszą wersję jako „wpis blogowy". Układ split: portret obok wprowadzenia, pasek trzech liczb (9 lat w firmie / 3 pokolenia / 36 lat ISABELL), dwa bloki, split z historią żakietów, kontakt z terminem 48 h. **Zero linkowania wewnętrznego** — jedynym wyjściem jest mail (decyzja Michała).
     - **Data w firmie: 2017** (nie 2023 — Michał sprostował, wcześniejsze zaangażowanie było realne).
     - **Cytat kanoniczny:** „Wolę zadać pięć pytań przed produkcją, niż tłumaczyć jedną pomyłkę na dwustu sztukach."
     - ⛔ **Zasada redakcyjna wymuszona przez Michała:** żadnych zaprzeczeń własnych kompetencji. Pierwsza wersja miała trzy („Nie szyję", „nie mamy dyplomów", „znają maszyny lepiej niż ja") — wszystkie wycięte. Celem jest pokazanie pozytywów, nie rozbrajanie zarzutów.
     - **Wątek MarTech/AI:** sekcja o technologii mówi o sklepach, automatyzacji i AI, ale **bez słowa „MarTech"** — dla marki odzieżowej to skrót z obcej branży. Kluczowa teza: zna produkcję z dwóch stron, bo sam prowadzi markę.
   - ✅ **ATRYBUCJA AUTORSTWA DOMKNIĘTA 09.08 w trzech miejscach:** (1) pięć najnowszych wpisów miało w schemacie `author: Organization` mimo nazwiska i avatara w nagłówku — teraz **25/25 to `Person`**; (2) nazwisko w nagłówku 25 wpisów **linkuje** do strony autora; (3) stopka na **43 stronach** ma podpis „Made by @mpjalbrzyk" → `https://www.mpjalbrzyk.pl/` (`target=_blank`, `rel=noopener`, **świadomie bez `nofollow`** — obie strony należą do Michała, link jest naturalny i ma zasilać jego stronę osobistą). Konwerter MDX załatany we wszystkich trzech punktach.
   - ✅ **KALKULATOR WYEKSPONOWANY 09.08 — baner w 15 wpisach** (było: 2 wpisy z linkiem w akapicie). Ciemny pasek z **wizualizacją interfejsu** (krok 2 kalkulatora, klocki Żakiet/Sukienka/Płaszcz/Kurtka z wyborem przeskakującym co 2 s, **czysty CSS bez JS**), nagłówek „Ile kosztuje uszycie Twojego modelu?", CTA „Policz swój model" i dwa mikro-dowody: **„Mniej niż minuta" + „Bez podawania maila"** (element wskazany przez Michała jako najmocniej podkręcający kliknięcie). Styl w `style.css`, jeden blok HTML na wpis.
     - **Dobór wpisów po audycie tematyki wszystkich 25.** Baner wchodzi tam, gdzie czytelnik realnie myśli o cenie lub ilości: koszty, MOQ, małe serie, próbka, Polska vs Chiny, opłacalność, 10 pytań, błędy, vs krawiec, jak znaleźć szwalnię, materiał na kurtki, powierzony materiał, tech pack, czas produkcji, CMT, lokalizacja.
     - ⛔ **Świadomie pominięte: 4 wpisy o odzieży firmowej** — mają **najwyższe sygnały cenowe na blogu**, ale kalkulator nie liczy bluz ani polo, więc kliknięcie kończyłoby się rozczarowaniem. Plus **5 wpisów czysto technicznych** (ściegi, stopniowanie, metkowanie, kontrola jakości, krojownia), gdzie czytelnik nie jest w trybie zakupowym.
     - ⛔ **`/ile-kosztuje-uszycie-partii-odziezy.html` bez banera** — ma własną ramkę z linkiem do kalkulatora na samej górze, dwa odwołania w jednym tekście to za dużo (decyzja Michała 09.08).
     - ⚠️ **Umieszczenie: WEWNĄTRZ sekcji, między akapitami.** Pierwsza wersja wstawiała baner przed następnym `<h2>` i w 8 wpisach lądował tuż pod grafiką — Michał odrzucił („po grafice natychmiast baner reklamowy"). Logika szuka teraz miejsca, gdzie przed banerem i po nim jest akapit.
     - 🐞 **Dwie pułapki złapane przy wdrożeniu:** (1) `.blog-post-body h3` ma specyficzność (0,1,1) i nadpisywał kolor nagłówka banera — **biały tytuł znikał na czarnym tle**; wszystkie selektory dostały prefiks `.kalk`. (2) Kolumna treści wpisu ma realnie **534 px, nie 720** — `max-width: 720px` zjada procentowy padding liczony od `<main>`, więc im szerszy ekran, tym węższy tekst. Układ poziomy ściskał tekst do 224 px, dlatego **baner jest zawsze pionowy**. Oba opisane komentarzem w `style.css`.

   ❄️ **NEWSLETTER I EMAIL MARKETING — ODŁOŻONE ŚWIADOMIE (decyzja 08.08).** Nie budować bazy ani nie stawiać formularza zapisu, **nawet pustego** (martwy element obniża zaufanie). Powody: przy 264 użytkownikach / 28 dni nawet 2% zapisu daje ~5 adresów miesięcznie; **cykl zakupowy B2B w szyciu jest krótki** (marka ma projekt i termin, potrzebuje wyceny teraz, nie pielęgnowania przez pół roku); kalkulator już pełni funkcję lead magnetu. **Próg powrotu do tematu: ~1000 użytkowników miesięcznie.** Jeśli pojawi się potrzeba zbierania adresów wcześniej, najtańsza droga to **opcjonalne „wyślij mi tę wycenę na maila" na końcu kalkulatora** (nie bramka, wynik nadal bez podawania danych) — daje adres z kontekstem wyrobu i kwoty. Listy polecanych firm nie robić, dopóki `/wspolpraca.html` nie przyprowadzi realnych partnerów, bo polecenie = odpowiedzialność za cudzą robotę.
...
7. ✅ Strona autora — **zrobiona 09.08** (patrz wyżej). 📋 Opinie Google (2 → kilkanaście) — zostaje.
